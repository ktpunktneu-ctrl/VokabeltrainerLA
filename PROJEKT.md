...
| Live-URL | https://ktpunktneu-ctrl.github.io/VokabeltrainerIT/ | https://ktpunktneu-ctrl.github.io/VokabeltrainerEN/ | — (Repo privat) | — (Repo privat) | — (Repo privat) | https://ktpunktneu-ctrl.github.io/VokabeltrainerLA/ |
| GitHub-Repo | ktpunktneu-ctrl/VokabeltrainerIT (öffentlich) | ktpunktneu-ctrl/VokabeltrainerEN (öffentlich) | ktpunktneu-ctrl/VokabeltrainerFR (privat) | ktpunktneu-ctrl/VokabeltrainerES (privat) | ktpunktneu-ctrl/VokabeltrainerBusinessEN (privat) | ktpunktneu-ctrl/VokabeltrainerLA (öffentlich) |
...
**VokabeltrainerLA — Besonderheiten (Stand 2026-08-15):** Dreistufiges Produktkonzept, angelehnt an die reale Struktur des Lateinunterrichts (Lehrbuchphase vs. Lektürephase), auf Klaus' Wunsch.

- **Stufe 1 (umgesetzt, live):** komplette App-Hülle wie bei FR/ES übernommen (Splash-Start, 3D-Kugel-Buttons, Lernbox, Quiz, PWA, Trial-Modell, OCR), 153 handkuratierte LA/DE-Wortpaare in 13 Kategorien.
- **Stufe 2 (umgesetzt, Anfänger/Lehrbuchphase):** Grammatik-Layer ergänzt.
  - **Substantive:** 63 von 66 Nomen im Wortschatz vollständig dekliniert (Nom/Gen/Dat/Akk/Abl, **ohne Vokativ**). 1./2. Deklination mit Singular+Plural (10 Formen), 3. Deklination bewusst **nur Singular** (5 Formen) — der Plural hat i-Stamm-Sonderfälle (Gen.Pl. `-um` vs. `-ium`), die sich nicht pauschal ohne Einzelprüfung sicher richtig herleiten lassen. 4./5. Deklination (`manus`, `dies`) und ein unregelmäßiges Wort (`bos`) bleiben unangetastet. Datenmodell: jedes Nomen bekommt `genus`/`deklination`/`genitiv` (Genitiv **explizit hinterlegt statt algorithmisch aus dem Nominativ abgeleitet** — fängt Sonderfälle wie `magister→magistri` vs. `puer→pueri` sauber ab, ohne pauschale Regeln zu riskieren), die vollen Formen werden daraus per Paradigma-Endungen berechnet (`deklinieren()` in `build_la_grammar.py`).
  - **Verben:** 22 reguläre Verben (1./2./3./3-io/4. Konjugation) per berechnetem Präsens-Paradigma + fest hinterlegten Stammformen (Perfekt/Supin), `esse`/`ire` als feste unregelmäßige Paradigmen. Deponens `mori` unangetastet.
  - **Adjektive/Farben** bleiben ohne Deklination (Stufe 3).
  - Neue UI: eigener "Deklination"-Modus (Quiz-Richtung + Lernbox-Tab) neben dem bestehenden "Verbformen"-Modus, beide hinter dem Trial-Lizenz-Gate wie gehabt. `PERSON_LABEL`/`ABFRAGBARE_FORMEN` (von ES übernommen, dort noch auf spanische yo/tu/el-Schlüssel bzw. eine vermutlich falsche `['past','participle']`-Auswahlliste gestellt — **in ES vermutlich ein bestehender Bug, dort nicht angefasst**) wurden für LA komplett neu auf Latein umgestellt (`ego/tu/is/nos/vos/ei`) und die fehlerhafte Zufallsauswahl durch `PERSONEN` ersetzt.
- **Stufe 3 (offen, Fortgeschrittene/Lektürephase):** 3.-Deklination-Plural inkl. i-Stamm-Regeln, 4./5. Deklination, Adjektivdeklination (inkl. unregelmäßiger Stämme wie `pulcher`/`niger`/`ruber`), Deponentien (`mori`), erweiterter Autoren-Wortschatz (Caesar/Cicero/Ovid-Niveau), komplexere Grammatik (AcI, Ablativus Absolutus, Konjunktiv). Als eigenständiges zweites Kaufprodukt gedacht (höherer Preis wegen Latinum-Relevanz), noch nicht begonnen.

**Gumroad-Produkt `vokabeltrainer-la` existiert noch nicht** — der Kaufen-Button im Lizenz-Modal verlinkt testweise dorthin, muss vor einem echten Verkaufsstart noch angelegt werden (Preisempfehlung: 9,95 € wie die anderen Apps).
